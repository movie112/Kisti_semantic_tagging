...
def train(model, train_dataloader, valid_dataloader, cfg, args):
    ...
    for epoch in range(args.epochs):
        ...
        for idx, items in enumerate(train_bar):
            ...
            preds_label_0, preds_label_1, _, _, _ = model(**item)
        ...
        with torch.no_grad():
            ...
            if best_val_loss > (sum(valid_epoch_loss)/len(valid_epoch_loss)):
                ...
                torch.save(model.state_dict(), cfg.DEMO_PATH+args.model_name)
            ...
            if best_val_f1 < f1_score(total_fine_labels, total_fine_preds, average='macro'):
                best_model_wts2 = copy.deepcopy(model.state_dict())
                torch.save(model.state_dict(), cfg.DEMO_PATH+'f1_'+args.model_name)
                print(f"{r_}Best F1-score Model was Saved!{sr_}")
                best_val_f1 = f1_score(total_fine_labels, total_fine_preds, average='macro')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--use_section', type=str, default='no_section', help='no_section, raw_section, idx_section')
    parser.add_argument('--load_model', type=str, default='lstmbert', help='lstmbert')
    parser.add_argument('--logger_file', type=str, default='/data/kisti/demo/demo_training.log', help='logger file name')
    parser.add_argument('--logger', type=str, default='final_results')
    parser.add_argument('--model_name', type=str, default='demo_model.bin')
    ...
    if args.demo == True:
        print(f"{g_}This is Demo Version!{sr_}")
        train_dataloader = pickle.load(open('/data/kisti/demo/demo_train_dataloader.pkl', 'rb'))
        dev_dataloader = pickle.load(open('/data/kisti/demo/demo_dev_dataloader.pkl', 'rb'))

    else:
    
        if args.plm == 'korscibert':
            if args.use_section == 'no_section':
                print(args.use_section)
                train_dataloader = pickle.load(open(cfg.DATA_SAVE_PATH+'korscibert_train_dataloader.pkl', 'rb'))
                dev_dataloader = pickle.load(open(cfg.DATA_SAVE_PATH+'korscibert_dev_dataloader.pkl', 'rb'))
            elif args.use_section == 'raw_section':
                print(args.use_section)
                train_dataloader = pickle.load(open(cfg.DATA_SAVE_PATH+'korscibert_osection_train_dataloader.pkl', 'rb'))
                dev_dataloader = pickle.load(open(cfg.DATA_SAVE_PATH+'korscibert_osection_dev_dataloader.pkl', 'rb'))
            elif args.use_section == 'idx_section':
                print(args.use_section)
                train_dataloader = pickle.load(open(cfg.DATA_SAVE_PATH+'korscibert_isection_train_dataloader.pkl', 'rb'))
                dev_dataloader = pickle.load(open(cfg.DATA_SAVE_PATH+'korscibert_isection_dev_dataloader.pkl', 'rb'))
            
        else:
            if args.use_section == 'no_section':
                print(args.use_section)
                train_dataloader = pickle.load(open(cfg.DATA_SAVE_PATH+'train_dataloader.pkl', 'rb'))
                dev_dataloader = pickle.load(open(cfg.DATA_SAVE_PATH+'dev_dataloader.pkl', 'rb'))
            elif args.use_section == 'raw_section':
                print(args.use_section)
                train_dataloader = pickle.load(open(cfg.DATA_SAVE_PATH+'section_train_dataloader.pkl', 'rb'))
                dev_dataloader = pickle.load(open(cfg.DATA_SAVE_PATH+'section_dev_dataloader.pkl', 'rb'))
            elif args.use_section == 'idx_section':
                print(args.use_section)
                train_dataloader = pickle.load(open(cfg.DATA_SAVE_PATH+'isection_train_dataloader.pkl', 'rb'))
                dev_dataloader = pickle.load(open(cfg.DATA_SAVE_PATH+'isection_dev_dataloader.pkl', 'rb'))
    ...
    logger.info(":::model:::{}, {}, {}:::".format(args.load_model, args.model_name, args.use_section))
